...
class DensityNet(object):
    ...
    def __init__(self, input_dim, output_dim, step_size):
        ...
        self._output = U.dense(h2, self.output_dim, weight_init=U.Xavier(1.0))

        net_params = tf.trainable_variables()
        grads = tf.gradients(self._output, net_params, -self.g_n)
    ...
    def eval(self, x_n) :
        ...
        return self._output.eval(feed, session=U.SESSION)
